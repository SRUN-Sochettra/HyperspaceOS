...
export default class Calculator extends BaseApp {
    ...
    handleKey(e) {
        ...
        const key = e.key
        if (key >= '0' && key <= '9') { e.preventDefault(); this.handleButton('num', key) }
        else if (key === '.') { e.preventDefault(); this.handleButton('num', '.') }
        else if (key === '+') { e.preventDefault(); this.handleButton('op', '+') }
        else if (key === '-') { e.preventDefault(); this.handleButton('op', '-') }
        else if (key === '*') { e.preventDefault(); this.handleButton('op', '*') }
        else if (key === '/') { e.preventDefault(); this.handleButton('op', '/') }
        else if (key === 'Enter' || key === '=') { e.preventDefault(); this.handleButton('equals', '') }
        else if (key === 'Escape') { e.preventDefault(); this.handleButton('clear', '') }
        ...
        else if (key === '%') { e.preventDefault(); this.handleButton('percent', '') }
    }
