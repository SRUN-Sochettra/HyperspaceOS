...
>>>>>>> origin/main
  ...
  renderTabs() {
    ...
    list.innerHTML = this.openFiles.map((file, i) => {
      const active = i === this.activeIndex
      const unsaved = this.unsavedChanges.has(file.path)
      const icon = this.getFileIcon(file.name)
      return `
        <div class="editor-tab ${active ? 'active' : ''}" data-idx="${i}">
          ...
          <span class="editor-tab-name">${escapeHtml(file.name)}${unsaved ? ' •' : ''}</span>
    ...
    const extensions = [
      ...
      EditorView.theme({
        ...
        ".cm-gutters": {
          ...
          color: "rgba(200,200,255,0.18)",
          minWidth: "40px",
        '.cm-activeLineGutter': { backgroundColor: 'rgba(0,245,255,0.05)', color: 'rgba(200,200,255,0.4)' },
        '.cm-activeLine': { backgroundColor: 'rgba(255,255,255,0.015)' },
        '.cm-cursor': { borderLeftColor: 'var(--neon-cyan)', borderLeftWidth: '2px' },
...
>>>>>>> origin/main
    ...
    if (saveEl) {
      ...
      this.addTimeout(() => {
        ...
        if (saveEl) { saveEl.textContent = ''; saveEl.style.color = '' }
      }, 2000)
    }
  }
  ...
  closeTab(index) {
    ...
    if (this.unsavedChanges.has(file.path)) {
      ...
      if (!confirm(`"${escapeHtml(file.name)}" has unsaved changes. Close anyway?`)) return
    }
    ...
    if (this.openFiles.length === 0) {
      ...
      this.renderTabs()
      const body = this.$(`#ed-body-${this.windowId}`)
      if (body) body.querySelectorAll('.cm-editor').forEach(el => el.remove())
      const empty = this.$(`#ed-empty-${this.windowId}`)
      if (empty) empty.style.display = ''
      this.updateStatusBar(null)
    } else {
...
>>>>>>> origin/main
  ...
  detectLang(name) {
    ...
    return map[ext] || "Plain Text";
      ...
      jsx: 'React', tsx: 'React TSX', json: 'JSON',
      css: 'CSS', html: 'HTML', md: 'Markdown',
      txt: 'Plain Text', log: 'Log', py: 'Python',
