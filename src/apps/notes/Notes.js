...
export default class Notes extends BaseApp {
  async setup() {
      ...
      <div class="notes-container">
        ...
        <div class="notes-editor">
          <div class="notes-toolbar">
            <button class="notes-tool-btn" id="notes-toggle-${this.windowId}" title="Toggle Preview" style="width:auto;padding:0 8px;font-size:var(--fs-xs)">Preview</button>
            ...
            <button class="notes-tool-btn" id="notes-delete-${this.windowId}" title="Delete Note" style="width:auto;padding:0 8px;font-size:var(--fs-xs)">Delete</button>
          </div>
    ...
    this.$(`#notes-toggle-${this.windowId}`).addEventListener('click', () => {
      ...
      this.previewEl.style.display = this.editMode ? 'none' : 'block'
      if (!this.editMode) {
        this.previewEl.innerHTML = marked.parse(this.textareaEl.value || '')
      }
    })
    ...
    this.$(`#notes-new-${this.windowId}`).addEventListener('click', () =>
      this.createNote(),
    )
    this.$(`#notes-delete-${this.windowId}`).addEventListener('click', () =>
      this.deleteNote(),
    )
