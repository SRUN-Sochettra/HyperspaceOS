...
export default class Terminal extends BaseApp {
  ...
  resolvePath(input) {
    ...
    if (input.startsWith("~/"))
      return FileSystem.join("/home/root", input.slice(2));
  ...
  executeCommand() {
    ...
    if (command) {
      try {
        command.execute({ args: cmdArgs, raw, terminal: this });
      } catch (err) {
        this.writeLine("error", `Error: ${err.message}`);
      }
    } else {
  ...
  autocomplete() {
    ...
    const dir = partial.includes("/")
      ? this.resolvePath(FileSystem.parentPath(partial))
      : this.cwd;
    const prefix = partial.includes("/")
      ? FileSystem.basename(partial)
      : partial;
    ...
    if (matches.length === 1) {
      ...
      parts[parts.length - 1] =
        (partial.includes("/") ? FileSystem.parentPath(partial) + "/" : "") +
        match.name +
        (match.type === "dir" ? "/" : "");
