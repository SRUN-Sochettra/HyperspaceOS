...
export const systemCommands = {
    ...
    execute({ terminal }) {
      import("./index.js").then(({ COMMANDS }) => {
        ...
        for (const [name, cmd] of Object.entries(COMMANDS)) {
          terminal.writeLine(
            "output",
            `  ${name.padEnd(maxLen + 2)} — ${cmd.description || ""}`,
          );
        }
        terminal.writeLine("output", "");
        terminal.writeLine(
    ...
    execute({ args, terminal }) {
      ...
      if (long) {
        ...
        for (let i = 0; i < filtered.length; i += cols) {
          ...
          terminal.writeLine(
            "output",
            row.map((e) => e.name.padEnd(maxLen)).join(""),
          );
        }
      }
    },
  ...
  cat: {
    ...
    execute({ args, terminal }) {
      if (!args[0]) {
        terminal.writeLine("error", "cat: missing operand");
        return;
      }
  ...
  touch: {
    ...
    execute({ args, terminal }) {
      if (!args[0]) {
        terminal.writeLine("error", "touch: missing operand");
        return;
      }
      const path = terminal.resolvePath(args[0]);
  ...
  mkdir: {
    ...
    execute({ args, terminal }) {
      if (!args[0]) {
        terminal.writeLine("error", "mkdir: missing operand");
        return;
      }
      const path = terminal.resolvePath(args[0]);
  ...
  rm: {
    ...
    execute({ args, terminal }) {
      ...
      if (targets.length === 0) {
        terminal.writeLine("error", "rm: missing operand");
        return;
      }
  ...
  mv: {
    ...
    execute({ args, terminal }) {
      if (args.length < 2) {
        terminal.writeLine("error", "mv: need source and destination");
        return;
      }
  ...
  cp: {
    ...
    execute({ args, terminal }) {
      if (args.length < 2) {
        terminal.writeLine("error", "cp: need source and destination");
        return;
      }
      const from = terminal.resolvePath(args[0]);
      const to = terminal.resolvePath(args[1]);
  ...
  du: {
    ...
    execute({ args, terminal }) {
      ...
      terminal.writeLine(
        "output",
        `Total size: ${formatBytes(usage.totalSize)}`,
      );
    },
  },
  ...
  head: {
    ...
    execute({ args, terminal }) {
      ...
      if (nFlag !== -1) {
        ...
        file = args.find(
          (a, i) => i !== nFlag && i !== nFlag + 1 && !a.startsWith("-"),
        );
      }
      if (!file) {
        terminal.writeLine("error", "head: missing file");
        return;
      }
      const content = FileSystem.readFile(terminal.resolvePath(file));
      if (content === null) {
        terminal.writeLine("error", `head: ${file}: not found`);
        return;
      }
      content
        .split("\n")
        .slice(0, lines)
        .forEach((l) => terminal.writeLine("output", l));
    },
  },
  ...
  tail: {
    ...
    execute({ args, terminal }) {
      ...
      if (!file) {
        terminal.writeLine("error", "tail: missing file");
        return;
      }
      const content = FileSystem.readFile(terminal.resolvePath(file));
      if (content === null) {
        terminal.writeLine("error", `tail: ${file}: not found`);
        return;
      }
      const allLines = content.split("\n");
      allLines
        .slice(Math.max(0, allLines.length - lines))
        .forEach((l) => terminal.writeLine("output", l));
    },
  },
  ...
  wc: {
    ...
    execute({ args, terminal }) {
      if (!args[0]) {
        terminal.writeLine("error", "wc: missing file");
        return;
      }
      const content = FileSystem.readFile(terminal.resolvePath(args[0]));
      if (content === null) {
        terminal.writeLine("error", `wc: ${args[0]}: not found`);
        return;
      }
      ...
      terminal.writeLine(
        "output",
        `  ${lines} lines  ${words} words  ${chars} chars  ${args[0]}`,
      );
    },
  },
  ...
  grep: {
    ...
    execute({ args, terminal }) {
      if (args.length < 2) {
        terminal.writeLine("error", "grep: usage: grep pattern file");
        return;
      }
      const pattern = args[0];
      ...
      if (content === null) {
        terminal.writeLine("error", `grep: ${args[1]}: not found`);
        return;
      }
  ...
  uname: {
    ...
    execute({ args, terminal }) {
      ...
      if (args.includes("-a")) {
        terminal.writeLine(
          "output",
          `HyperSpace OS v${v} — Build ${Store.get("os.buildDate")}`,
        );
        terminal.writeLine(
          "output",
  ...
  whoami: {
    ...
    execute({ terminal }) {
      terminal.writeLine("output", "root@hyperspace");
    },
  },
  hostname: {
    ...
    execute({ terminal }) {
      terminal.writeLine(
        "output",
        FileSystem.readFile("/etc/hostname") || "hyperspace",
      );
    },
  },
  ...
  neofetch: {
    ...
    execute({ terminal }) {
      ...
      terminal.writeLine(
        "output",
        `                   Files: ${du.fileCount} (${formatBytes(du.totalSize)})`,
      );
  ...
  env: {
    ...
    execute({ terminal }) {
      const vars = {
        USER: "root",
        HOME: "/home/root",
        SHELL: "/bin/hyper-bash",
        TERM: "hyperspace-256color",
  ...
  ps: {
    ...
    execute({ terminal }) {
      ...
      if (windows.length === 0) {
        terminal.writeLine("output", "  (no windows)");
        return;
      }
      for (const w of windows) {
        terminal.writeLine(
          "output",
          `  ${String(w.id).padStart(4)}   ${w.appId.padEnd(16)} ${(w.minimized ? "minimized" : w.maximized ? "maximized" : "running").padEnd(11)} ${w.width}x${w.height}`,
        );
      }
    },
  },
  ...
  kill: {
    ...
    execute({ args, terminal }) {
      ...
      if (isNaN(pid)) {
        terminal.writeLine("error", "kill: usage: kill [pid]");
        return;
      }
      import("../../../core/EventBus.js").then(({ default: EB }) => {
        ...
        if (w) {
          EB.emit("window:close", { id: pid });
          terminal.writeLine("success", `Killed ${pid} (${w.appId})`);
  ...
  calc: {
    ...
    execute({ raw, terminal }) {
      ...
      if (!expr || expr === "calc") {
        terminal.writeLine("error", "calc: usage: calc [expr]");
        return;
      }
      try {
        terminal.writeLine(
        ...
        terminal.writeLine("error", "Invalid expression");
      }
    },
  },
  ...
  history: {
    ...
    execute({ terminal }) {
      ...
      terminal.history.forEach((cmd, i) =>
        terminal.writeLine("output", `  ${String(i + 1).padStart(4)}  ${cmd}`),
      );
    },
  },
  ...
  free: {
    ...
    execute({ terminal }) {
      ...
      terminal.writeLine(
        "output",
        `Mem:         ${mem}GB    ${used}GB   ${(mem - parseFloat(used)).toFixed(1)}GB`,
      );
    },
  },
