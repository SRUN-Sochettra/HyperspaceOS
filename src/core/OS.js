...
async function registerApps() {
  ...
  modules.forEach(mod => {
    ...
    else if (typeof mod.register === 'function') mod.register()
  })
}
...
>>>>>>> origin/main
  ...
  try {
    ...
    const { default: ClipboardManager } =
      await import("../ui/ClipboardManager.js");
    const { default: DragDropController } =
      await import("../wm/DragDropController.js");
...
=======
    ...
    setTimeout(async () => {
      ...
      const restored = await Persistence.apply()
      const { default: WindowManager } = await import('../wm/WindowManager.js')
      if (!restored && WindowManager.getAllWindows().length === 0) {
        Registry.launch('terminal')
      } else if (restored) {
        EventBus.emit('notification:show', {
          icon: '',
    ...
    if (bs) {
      const el = document.createElement("div");
      el.style.cssText =
        "color:#ff5f57;font-size:12px;margin-top:20px;text-align:center;max-width:400px;";
      el.textContent = `Boot failed: ${err.message}`;
      bs.appendChild(el);
>>>>>>> origin/main
      const el = document.createElement('div')
