...
const SnapController = (() => {
  ...
  function showPreview(zoneName) {
    ...
    if (zoneName === "top") {
      ...
      left = zone.x * w + pad;
      top =
        (typeof zone.y === "number" && zone.y < 1
          ? zone.y === 32
            ? statusH
            : zone.y * availH + statusH
          : statusH) + pad;
  ...
  function resolve() {
    ...
    switch (zone) {
      case "top":
        bounds.x = pad;
        bounds.y = statusH + pad;
        bounds.width = w - pad * 2;
        bounds.height = availH - pad * 2;
        break;
      case "left":
        bounds.x = pad;
        bounds.y = statusH + pad;
        bounds.width = w / 2 - pad * 1.5;
        bounds.height = availH - pad * 2;
        break;
      case "right":
        bounds.x = w / 2 + pad * 0.5;
        bounds.y = statusH + pad;
        bounds.width = w / 2 - pad * 1.5;
        bounds.height = availH - pad * 2;
        break;
      case "topLeft":
        ...
        bounds.y = statusH + pad;
        bounds.width = w / 2 - pad * 1.5;
        bounds.height = availH / 2 - pad * 1.5;
        break;
      case "topRight":
        bounds.x = w / 2 + pad * 0.5;
        bounds.y = statusH + pad;
      ...
      case "bottomLeft":
        bounds.x = pad;
        bounds.y = statusH + availH / 2 + pad * 0.5;
        bounds.width = w / 2 - pad * 1.5;
        bounds.height = availH / 2 - pad * 1.5;
        break;
      case "bottomRight":
