...
const WindowManager = (() => {
  ...
  function tileAll() {
    ...
    visible.forEach((win, i) => {
      ...
      win.setPosition(col * cellW + pad, areaTop + row * cellH + pad);
      win.setSize(cellW - pad * 2, cellH - pad * 2);
    });
  ...
  function syncStore() {
    Store.set(
      "windows.all",
      [...windows.values()].map((w) => w.toJSON()),
    );
    Store.set("apps.open", [
      ...new Set(
        [...windows.values()].filter((w) => !w.destroyed).map((w) => w.appId),
      ),
    ]);
  }
